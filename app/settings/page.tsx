...
export default function SettingsPage() {
    ...
    // Initialize org ID on client side
    useEffect(() => {
        setOrgId(getOrCreateOrgId());
    }, []);
                            ...
                            <div className="bg-[rgb(var(--surface))] rounded-xl border border-[rgb(var(--border))] shadow-sm p-6">
                                ...
                                <div className="space-y-5">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {WIDGET_COLORS.map(color => (
                                                <button
                                                    ...
                                                    className={`w-9 h-6 rounded-lg border-2 transition-smooth ${widgetColor === color.id
                                    ...
                                    <div className="grid md:grid-cols-[220px_1fr] gap-3 md:gap-4 items-start">
                                        ...
                                        <div className="flex p-1 bg-[rgb(var(--surface-muted))] rounded-full w-fit">
                                            {WIDGET_SIZES.map(size => (
                                                <button
                                                    key={size.id}
                                                    onClick={() => setWidgetSize(size.id)}
                                                    className={`px-4 py-2 text-sm font-medium rounded-full transition-smooth ${widgetSize === size.id
